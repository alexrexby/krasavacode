...
const FIRST_PROJECTS = [
  ...
  },
  {
    ...
    // Затравка для "Свой проект": явный императив "реализуй файлами" чтобы
    // слабые модели не отвечали туториалом. Сам promptик показывает в чате
    // что от ученика ждут описания идеи.
    prompt:
      'Сейчас ученик напишет свою идею проекта одним сообщением. ' +
      'Твоя задача: СРАЗУ реализовать её — создать новую подпапку с понятным именем и положить туда все нужные файлы через инструмент Write. ' +
      'Не пиши инструкции "вам нужно создать файл X". Не предлагай шаги. Просто СОЗДАВАЙ ФАЙЛЫ. ' +
      'Если идея совсем неясна — задай ОДИН уточняющий вопрос. После ответа — сразу делай. ' +
      'Сейчас просто скажи ученику одной строкой по-русски: "Опиши идею — что хочешь сделать? Я сразу начну реализовывать в файлах, без долгих объяснений."',
  },
];
...
<body>
  <div class="card">
    ...
    <p class="sub"><b>Минимум — один провайдер</b>, для начала достаточно. Если подключишь оба — при ошибке одного автоматом переключится на второй.</p>
    <div class="ru-banner">
      📍 <b>Без VPN из РФ/Беларуси</b> работает только <b>🇷🇺 Polza.ai</b> (платный, рубли, ~100₽ на сотни запросов). <b>OpenRouter</b> бесплатный (50 запросов/день), но требует VPN — многие IP из РФ он не пускает.
    </div>
...
async function cliOnboarding() {
  ...
  if (have.length === 0) {
    console.log('\n  Ни одного провайдера не подключено — без них AI работать не сможет.');
    console.log('  Запусти мастер ещё раз: krasavacode setup\n');
  }
